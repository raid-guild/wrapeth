...
import { w3modal, providerOptions, createWeb3User } from "../utils/Auth";
...
const Store = ({ children }) => {
  ...
  useEffect(() => {
    ...
    web3Modal?.web3?.currentProvider &&
      web3Modal.web3.currentProvider
        .on("accountsChanged", accountsChanged)
        .on("chainChanged", handleChainChange);

    const unsub = () => {
      if (web3Modal?.web3?.currentProvider && web3Modal?.cachedProvider) {
  ...
  useEffect(() => {
    const initContract = async () => {
      ...
      try {
        const contract = new web3Modal.web3.eth.Contract(
          WethAbi,
          wethAddrs[getChainData(+network).network]
        );
